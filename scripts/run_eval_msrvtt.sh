...
for IDX in $(seq 0 $((CHUNKS-1))); do
  CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 ${ROOT_DIR}/run_inference_video_qa.py \
      --video_dir ${DATA_DIR}/MSRVTT_Zero_Shot_QA/videos/all \
      --gt_file_question ${GT_QA_DIR}/MSRVTT_Zero_Shot_QA/val_q.json \
      --gt_file_answers ${GT_QA_DIR}/MSRVTT_Zero_Shot_QA/val_a.json \
      --output_dir ${OUTPUT_DIR}/MSRVTT_Zero_Shot_QA/${CONFIG_NAME} \
...
temp_dir=${TEMP_DIR}/MSRVTT_Zero_Shot_QA/${CONFIG_NAME}
