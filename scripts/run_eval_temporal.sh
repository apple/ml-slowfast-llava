...
for IDX in $(seq 0 $((CHUNKS-1))); do
	 CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 ${ROOT_DIR}/run_inference_generative_qa.py \
		 ...
		 --gt_file_question ${GT_QA_DIR}/VCGBench/Video-ChatGPT-temporal_understanding_val_qa.json \
		 --gt_file_answers ${GT_QA_DIR}/VCGBench/Video-ChatGPT-temporal_understanding_val_qa.json \
		 --output_dir ${OUTPUT_DIR}/Video_ChatGPT/temporal_understanding/${CONFIG_NAME} \
...
temp_dir=${TEMP_DIR}/Video_ChatGPT/temporal_understanding/${CONFIG_NAME}
...
python3 ${ROOT_DIR}/eval/eval_generative_qa.py \
		...
		--output_json ${output_dir}/results_temporal_understanding.json \
