...
def main(args, task_name="MSVD_Zero_Shot_QA"):
    data_q = []
    data_a = []
    ...
    with open(f"playground/gt_qa_files/{task_name}/val_q.json", "w") as f:
        json.dump(data_q, f, indent=4)
    with open(f"playground/gt_qa_files/{task_name}/val_a.json", "w") as f:
...
def parse_args():
    ...
    parser.add_argument("--qa_file", help="Path to MSVD_QA.csv", required=True)
