...
def main(args, task_name="Activitynet_Zero_Shot_QA"):
    data_q = []
    data_a = []
    ...
    with open(f"{folder}/test_q.json", "w") as f:
        json.dump(data_q, f, indent=4)
    with open(f"{folder}/test_a.json", "w") as f:
...
def parse_args():
    ...
    parser.add_argument("--qa_file", help="Path to Activitynet_QA.csv", required=True)
