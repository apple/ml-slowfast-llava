...
for IDX in $(seq 0 $((CHUNKS-1))); do
  CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 ${ROOT_DIR}/run_inference_multiple_choice_qa.py \
      --video_dir ${DATA_DIR}/EgoSchema/video \
      --gt_file ${GT_QA_DIR}/EgoSchema/val_qa.json \
      --output_dir ${OUTPUT_DIR}/EgoSchema/${CONFIG_NAME} \
...
temp_dir=${TEMP_DIR}/EgoSchema/${CONFIG_NAME}
